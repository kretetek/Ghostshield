...
//		alert((maskWidth-768)/2+768);
		if( maskWidth<=768)
			wid=768;
		else if (maskWidth>768 && maskWidth<=1000)
			wid=1000;
		else if (maskWidth>1572)
			wid=1170;
		else
			wid=(maskWidth-768)/2+768;
		$('.zn_responsive .ca-container').css('width',wid);
		lef=Math.ceil(wid/3);
		$('.zn_responsive .ca-item').css('width',lef-4);
		
		last=$('.zn_responsive .ca-item').length+1;
		for (i=1;i<last;i++)
		{
			if($('.ca-item-'+i).length)
			{
				if (parseInt($('.zn_responsive .ca-item-'+i).css('left'),10)==0)
					break;
			}
		}
		if(i!=last)
		{
			var factor;
			if (parseInt($('.zn_responsive .ca-item-' + i).find('div.ca-content-wrapper').css('width'),10)!=0)
			{
				$('.zn_responsive .ca-item-' + i).find('div.ca-content-wrapper').css('left',lef+'px');
				$('.zn_responsive .ca-item-' + i).find('div.ca-content-wrapper').css('width',(lef*2)+'px');
				$('.zn_responsive .ca-item-' + i).find('div.ca-content-wrapper').find('div.ca-content').css('width',(lef*2-20)+'px');
			}
			
			if ($('.zn_responsive .ca-item-' + i).find('div.ca-content-wrapper').css('width')=='0px')
				factor=1;
			else
				factor=3;
			for (j=1;j<last;j++)
			{
				if ($('.zn_responsive .ca-item-'+j).length)
				{
					var k;
					if(j<i) k=j-i+(last-1); else k=j-i;
					$('.zn_responsive .ca-item-'+j).css('left',lef*k*factor);
				}
			}
		}
	}
