...
if (!isset($_REQUEST["dropzone"])) {
	//db::debug();
	echo $page->header("Commercial // Gallery", "commercial"); //require_once("inc/header.php");
	...
	<section id="gallery">
		<h1>Commercial Project Gallery</h1>
		<ul class="text_menu">
			<li>'.((!isset($_REQUEST["sort"]))?'<span>By Date</span>':'<a href="/commercial/gallery/">By Date</a>').'
			<li>'.((isset($_REQUEST["sort"]) && $_REQUEST["sort"] == "product")?'<span>By Product</span>':'<a href="/commercial/gallery/product/">By Product</a>').'
			<!--li>'.((isset($_REQUEST["sort"]) && $_REQUEST["sort"] == "date")?'<span>By Calendar</span>':'<a href="/commercial/gallery/date/">By Calendar</a>').'-->
		</ul>';
	...
	if (!isset($_REQUEST["sort"])){
		$gallery_items = db::_query('SELECT * FROM `gallery'.($page->dev?"-dev":"").'` WHERE ('.$limit["commercial"].') AND `_approved`=1 ORDER BY `_created` DESC');
		if (sizeof($gallery_items) > 0) echo $gallery->create($gallery_items, "Default", "commercial");
		...
		for ($y = $beg_y; $y >= $beg_y; $y--) {
			...
			for ($m = $cur_m; $m >= $beg_m; $m--) {
				...
				$gallery_items = db::_query('SELECT * FROM `gallery'.($dev?"-dev":"").'` WHERE ('.$limit["commercial"].') AND `_approved`=1 AND `_created` >= "'.$current.' 00:00:00" AND `_created` < "'.$next.' 00:00:00" ORDER BY `_created` DESC');
				
				if (sizeof($gallery_items) > 0) {
					$name = date("F Y", strtotime($current));
					echo '<h3>'.$name.'</h3>';
					echo $gallery->create($gallery_items, $name, "commercial", false, false);
				}
	...
	echo '
					...
					<p><small>This email will only be used in regard to shared media.</small></p>
				</div>
			</div>
