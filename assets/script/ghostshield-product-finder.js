...
function handle_product_finder() {
	$('#product_finder a').click(function(ev){
		if (!$(this).hasClass("link_out")) {
			ev.preventDefault();
			var hash = "#" + $(this).attr("href").replace("?", "").replace("/product-finder/", "").replace("&ajax", "").replace(/\&q[0-9]+\=/g, "/").replace(" ", "+").toLowerCase();
...
function product_finder_go(href) {
	$.ajax({ method:"GET", url:href, data:{ajax: "true"} }).done(function(msg){
		...
		var offset = (document.getElementById("product_finder_anchor") ? document.getElementById("product_finder_anchor").offsetTop : 0) + 150;
