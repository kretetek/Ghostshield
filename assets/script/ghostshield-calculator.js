$(function(){
	$('.calculator .calculation').html('<table class="placeholder"><tr><td>*<td>Coverage may vary due to porosity and texture of substrate.</table>');
	$('.calculator input#project_sqft').input(calculator);
	//$('.calculator .formula input').change(calculator);
});

function calculator() {
	$.ajax({ method:"POST", url:"/sqftcalculate.php", data:{
		project_coverage: $('#project_coverage').val(),
		project_coats: $('#project_coats').val(),
		project_model: $('#project_model').val(),
		project_containers: $('#project_containers').val(),
		project_concentrate: $('#project_concentrate').val(),
		project_sqft: $(this).val()
	}}).done(function(msg){
		$('.calculator .calculation').html(/*'<label class="formula" for="project_sqft">&nbsp;</label>'+*/msg);
	});
}
